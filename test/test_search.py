...
report = classification_report(y_true_t==1, torch.tensor(y_prob_val)>=thresholds, digits=4, output_dict=True, zero_division=0)
report_df = pd.DataFrame(report).transpose()
report_df.to_csv(f'./module_3/output/classification_report_r{best_round}.csv')
report_print = classification_report(y_true_t==1, torch.tensor(y_prob_val)>=thresholds, digits=4, target_names=labels_col, zero_division=0)
print("classification report:")
print(report_print)

report_with_auc = roc_auc_score(
    y_true=y_true_t.numpy(),  
    y_score=y_prob_t.numpy(),
    average=None)

list_report = report_with_auc.tolist()
list_report += [sum(list_report)/len(list_report)]
max_f1s = [report[str(i)]['f1-score'] for i in range(15)] + [report['macro avg']['f1-score']]
