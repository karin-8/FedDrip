...
def main(args):
    ...
    for i in range(start_round, num_rounds):
        if i+1 >= args.lr_decay_start:
            logger.info("Decaying learning rate from %f to %f" % (server.lr, server.lr*args.lr_decay_per_round))
            server.lr = server.lr*args.lr_decay_per_round
            # print(server.lr)
        
        if args.ps_warmup_rounds != 0 and  i < args.ps_warmup_rounds:
            server.client_weights[-1] = args.client_weights[-1]*(i/args.ps_warmup_rounds)
            logger.info(f"Pseudo-site's weight is {server.client_weights[-1]:.4f}")
        ...
        ##### Test model #####
        if eval_every != 0 and ((i + 1) % eval_every == 0 or (i + 1) == num_rounds): # or (i+1) > num_rounds - 100:  # eval every round in last 100 rounds
            criterion = nn.BCELoss().to(args.device)
            loss, auc = evaluate(i+1, server.client_model, val_loader, criterion, args)
            logger.info(f"Validation Loss = {loss:.4f}, Validation AUC = {auc:.4f}")
            ...
            if not os.path.isfile(os.path.join("module_3", args.work_dir, "losses.json")):
                ...
                with open(os.path.join("module_3", args.work_dir, "losses.json"), "r") as f:
                    d = json.load(f)
                if i == math.ceil((start_round+1)/eval_every)*eval_every - 1:
                    d["loss"] = d["loss"][:start_round//eval_every]
                    d["auc"] = d["auc"][:start_round//eval_every]
                    # print(d)
                d["loss"].append(loss)
                d["auc"].append(auc)
                with open(os.path.join("module_3", args.work_dir, "losses.json"), "w") as f:
