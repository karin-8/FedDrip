...
parser.add_argument("--alpha_coef", type=float, default=1e-3)
...
def main(train_args):
    ...
    if train_args.train == 'true':
        with open(logfile, "a") as f:
            ...
            local_model = ClientModel(*model_params, args.device)
            ...
            local_model.load_state_dict(torch.load(os.path.join("module_3", args.work_dir, f"local_model_site_{train_args.client}.ckpt"))["state_dict"])
            
            cloud_model_param = get_mdl_params([cloud_model])[0]
            local_model_param = get_mdl_params([local_model])[0]
            
            cloud_model_param_tensor = torch.tensor(cloud_model_param, dtype=torch.float32, device=args.device)
            local_model_param_tensor = torch.tensor(local_model_param, dtype=torch.float32, device=args.device)
            ...
            update, f1, val_losses = train(model, cloud_model_param_tensor, local_model_param_tensor, trainloader, testloader, train_args, max_norm, f)
...
def train(model, cloud_model, local_model, trainloader, testloader, train_args, max_norm, f):
    ...
    for epoch in range(n_epochs):
        model.train()
        losses[epoch] = run_epoch(epoch, model, cloud_model, local_model, trainloader, optimizer, criterion, train_args, max_norm, f)
...
def run_epoch(epoch, model, cloud_model, local_grad_vector, trainloader, optimizer, criterion, train_args, max_norm, f):
    ...
    for j, data in enumerate(trainloader):
        ...
        loss_algo = train_args.alpha_coef * torch.sum(local_par_list * (-cloud_model + local_grad_vector))
