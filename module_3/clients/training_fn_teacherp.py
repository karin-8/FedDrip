...
def main(train_args):
    ...
    if train_args.train == 'true':
        with open(logfile, "a") as f:
            ...
            if train_args.round > 0 and args.pseudo_site_ids:
                ...
                cloud_model.load_state_dict(torch.load(os.path.join("module_3", args.work_dir, f"local_model_site_{args.pseudo_site_ids[0]}.ckpt"))["state_dict"])
            ...
            update, f1, val_losses = train(model, cloud_model, trainloader, testloader, train_args, max_norm, f)
...
def train(model, cloud_model, trainloader, testloader, train_args, max_norm, f):
    ...
    if train_args.pseudo == 1:
        ...
        distil_alpha = 0.9  # Weight for distillation loss
    ...
    for epoch in range(n_epochs):
        model.train()
        if train_args.pseudo == 1 or train_args.round == 0 or not args.pseudo_site_ids:
            losses[epoch] = run_epoch(epoch, model, cloud_model, trainloader, optimizer, criterion, train_args, max_norm, f)
            # losses[epoch] = 0.5
        elif args.pseudo_site_ids:
            losses[epoch] = run_epoch_distil(epoch, model, cloud_model, trainloader, optimizer, criterion, 
                          criterion_distillation, distil_alpha, distil_temperature, train_args, max_norm, f)
            
        f.write(f"Training epoch {epoch}/{train_args.n_epochs}\n")
        f.flush()
        val_losses[epoch], f1 = evaluate(epoch, model, testloader, criterion, train_args, f)
...
def run_epoch(epoch, model, cloud_model, trainloader, optimizer, criterion, train_args, max_norm, f, pseudo_site=False):
...
def run_epoch_distil(epoch, model, cloud_model, trainloader, optimizer, criterion, criterion_distillation, distil_alpha, 
                     train_args, max_norm, f, pseudo_site=False):
    ...
    for j, data in enumerate(trainloader):
        ...
        with torch.no_grad():  # No gradient for teacher model
            teacher_outputs = cloud_model(input_data_tensor)
...
def evaluate(epoch, model, testloader, criterion, train_args, f):
    with torch.no_grad():
        running_loss = 0
        i = 0
        y_prob = []
