...
from functions.custom_transforms import ApplyTokenizerd, LoadJSONd, RandomSelectExcerptd
...
def get_datalist(
    ...
    for index, row in df.iterrows():
        report = get_report(prompt_path, df, row)
        img_path = "/karin/project/fdl/data/" + row["Image Index"]
    ...
    print(f"{name} Data: Found {len(data_dicts)} subjects.")
    return data_dicts
...
def get_dataloader(
    ...
    if eval_mode == "loss":
        val_transforms = transforms.Compose(
            [
                ...
                transforms.Resized(keys=["image"], spatial_size=(img_size, img_size)),
    ...
    if len(train_df) != 0:
        train_dicts = get_datalist(name='Train', df=train_df, prompt_path=prompt_path)
        train_ds = PersistentDataset(data=train_dicts, transform=train_transforms, cache_dir=cache_dir)
        ...
        train_loader = None
    val_dicts = get_datalist(name='Validation', df=val_df, prompt_path=prompt_path,)
    val_ds = PersistentDataset(data=val_dicts, transform=val_transforms, cache_dir=cache_dir)
